...
publishLocalRegistry version:
  ...
  creds="$(printf 'protocol=https\nhost=gitea.local.vgerasimov.dev\n\n' | git credential fill)"; user="$(printf '%s\n' "$creds" | sed -n 's/^username=//p')"; token="$(printf '%s\n' "$creds" | sed -n 's/^password=//p')"; test -n "$user"; test -n "$token"; VERSION="{{version}}" GITEA_USERNAME="$user" GITEA_TOKEN="$token" sbt clean test publish
