...
class ParserTest extends TemploTestSuite(Parser()):
    ...
    testOne("text 1 {{ code }} text 2 -> code surrounded with text") {
    ...
    testOneError("unclosed {{ expression returns parser error") {
        "before {{(+ 1 2)" -> Parser.Error("Unclosed template expression")
    }
